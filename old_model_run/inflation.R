...
# save(ppp_exchange, file="old_model_run/data_all/ppp_exchange.RData")
load("old_model_run/data_all/ppp_exchange.RData")
### local currency units to USD
# wb_exchange <- wb_data(indicator=c("PA.NUS.FCRF"))
# wb_exchange <- as.data.table(wb_exchange)
# save(wb_exchange, file="old_model_run/data_all/wb_exchange.RData")
load("old_model_run/data_all/wb_exchange.RData")


# ### inflation data creation ####### # last downloaded 07/10/2020
# gdpdeflator_data <- wb_data(indicator = "NY.GDP.DEFL.ZS")
# # # save(gdpdeflator_data, file="old_model_run/data_all/wb_GDP_deflator.RData")
# inflation_souce <- as.data.table(gdpdeflator_data)
# save(inflation_source, file="old_model_run/data_all/inflation_source.RData")
load("old_model_run/data_all/inflation_source.RData")


# ## match countries with currencies
# currency_country <- read.csv("old_model_run/data_all/currency_country_adj.csv")
...
#                                        !is.na(number)]
# # save(currency_country, file="old_model_run/data_all/currency_country.RData")
load("old_model_run/data_all/currency_country.RData")
...
euro_wb_exc <- read.csv("old_model_run/data_all/PA_NUS_FCRF_20210312.csv") %>%
                  as.data.table()
...
euro_wb_gdp <- read.csv("old_model_run/data_all/NY_GDP_DEFL_KD_ZG_EUR-20210325.csv") %>%
  as.data.table()
...
eur_ppp <- read.csv("old_model_run/data_all/EUR_PPP.csv") %>%
              as.data.table()
